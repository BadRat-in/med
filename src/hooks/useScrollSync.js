...
function anchorsIn(body) {
  ...
  return [...body.querySelectorAll('[data-line]')].sort(
    (a, b) => parseInt(a.getAttribute('data-line'), 10) - parseInt(b.getAttribute('data-line'), 10)
  )
}
...
export function scrollPreviewToLine(viewport, body, line) {
  ...
  let i = 0
  while (i + 1 < anchors.length && parseInt(anchors[i + 1].getAttribute('data-line'), 10) <= line) {
    i += 1
  }
  ...
  const frac =
    lineEnd > lineStart ? Math.max(0, Math.min(1, (line - lineStart) / (lineEnd - lineStart))) : 0
...
export function lineFromPreview(viewport, body) {
  ...
  const next = anchors[i + 1]
  const lineStart = parseInt(cur.getAttribute('data-line'), 10)
  const lineEnd = next ? parseInt(next.getAttribute('data-line'), 10) : lineStart + 10
